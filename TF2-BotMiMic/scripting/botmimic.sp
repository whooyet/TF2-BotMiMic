...
public void OnPlayerRunCmdPost(int client, int buttons, int impulse, const float vel[3], const float angles[3], int weapon, int subtype, int cmdnum, int tickcount, int seed, const int mouse[2])
{
	...
	iFrame[newWeapon] = GetClientSlot(client, GetItemIndex(client)); 
	iFrame[newWeaponindex] = GetItemIndex(client);
	iFrame[playerteam] = GetClientTeam(client);
	iFrame[playerclass] = TF2_GetPlayerClass(client);
	...
	if(iNewWeapon != -1)
	{
		...
		if(IsValidEntity(iNewWeapon) && IsValidEdict(iNewWeapon))
		{
			...
			int weaponId = GetEntProp(iNewWeapon, Prop_Send, "m_iItemDefinitionIndex");
			
			iFrame[newWeaponindex] = weaponId;
		}
	}
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
	...
	if(TF2Econ_GetItemSlot(GetItemIndex(client), TF2_GetPlayerClass(client)) != iFrame[newWeapon])
	{
		ChangePlayerWeaponSlot(client, TF2Econ_GetItemSlot(iFrame[newWeaponindex], TF2_GetPlayerClass(client)));
		// PrintToChatAll("%d  %d", TF2Econ_GetItemSlot(GetItemIndex(client), TF2_GetPlayerClass(client)), iFrame[newWeapon]);
	}
	if(GetItemIndex(client) != iFrame[newWeaponindex])
	{
		char iClassName[PLATFORM_MAX_PATH];
		TF2Econ_GetItemClassName(iFrame[newWeaponindex], iClassName, sizeof(iClassName));
		CreateWeapon(client, iClassName, TF2Econ_GetItemSlot(iFrame[newWeaponindex], TF2_GetPlayerClass(client)), iFrame[newWeaponindex]);
	}
	
	if(GetClientTeam(client) != iFrame[playerteam]) ChangeClientTeamAlive(client, iFrame[playerteam]);
	if(TF2_GetPlayerClass(client) != iFrame[playerclass]) SetClass(client, iFrame[playerclass]);
	
	/* Log */
	
	// char iClassName[PLATFORM_MAX_PATH];
	// TF2Econ_GetItemClassName(iFrame[newWeaponindex], iClassName, sizeof(iClassName));
	// PrintToChatAll("\x03%s", iClassName);
...
public void Event_OnPlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	else if(g_hBotMimicsRecord[client] != null)
	{
		...
		g_iBotMimicTick[client] = 0;
		g_iCurrentAdditionalTeleportIndex[client] = 0;
		if(g_hCVRespawnOnDeath.BoolValue)
		{
			// CreateTimer(1.0, Timer_DelayedRespawn, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
			Handle hTemp;
			CreateDataTimer(0.3, SpawnBot, hTemp, TIMER_FLAG_NO_MAPCHANGE);

			WritePackCell(hTemp, client);
			WritePackCell(hTemp, TF2_GetPlayerClass(client));
		}
	}
}
...
public Action Timer_DelayedRespawn(Handle timer, any userid)
{
	...
	if(g_hBotMimicsRecord[client] != null && IsClientInGame(client) && !IsPlayerAlive(client) && IsFakeClient(client))
		TF2_RespawnPlayer(client);
	
	return Plugin_Stop;
}

public Action SpawnBot(Handle hTimer, Handle hPack)
{
	ResetPack(hPack);
	int client = ReadPackCell(hPack);
	int class = ReadPackCell(hPack);
...
public int SaveBookmark(Handle plugin, int numParams)
{
	...
	int iWeapon = Client_GetActiveWeapon(client);
	if(iWeapon != INVALID_ENT_REFERENCE && iFrame[newWeapon] == GetClientSlot(client, GetItemIndex(client)) && IsValidEntity(iWeapon))
	{
		int weaponId = GetEntProp(iWeapon, Prop_Send, "m_iItemDefinitionIndex");
		iFrame[newWeaponindex] = weaponId;
	}
...
stock void CreateWeapon(int client, char[] classname, int slot, int itemindex)
{
