...
{
	for(int i = 1; i <= MaxClients; i++) if(IsClientConnected(i) && IsClientInGame(i)) SDKHook(i, SDKHook_OnTakeDamage, OnTakeDamage);
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3])
{
	...
	if(victim == attacker) if(IsFakeClient(victim)) TF2_AddCondition(victim, view_as<TFCond>(14), 0.001);
